...
Tree::Tree(DSM *dsm, uint16_t tree_id) : dsm(dsm), tree_id(tree_id){
  ...
  for (int i = 0; i < dsm->getClusterSize(); ++i) {
    local_locks[i] = new LocalLockNode[define::kNumOfLock];//这个i是针对集群内的节点的，每个local_lock[i]都指向一个数组，数组大小为KNumOfLock
  ...
  dsm->write_sync(page_buffer, root_addr, kLeafPageSize);  //将page_buffer实例化为root_page，填充好首位version，然后写入远端，先前分配的root_addr
...
// Maybe it will goes to the sibling pointer or go to the children
  if (!result.is_leaf) {  //这里的if分支表示找到的result不是叶节点，如果是叶节点，之间进行leaf_page_store就可以了
    assert(result.level != 0);
    ...
    p = result.next_level;  //如果不存在sibling节点，就继续去往下一层
...
re_read:
  ...
  dsm->read_sync(page_buffer, page_addr, kLeafPageSize, cxt);//从page_buffer里读page_addr指示得地址上的内容
...
//        printf("should turn right ");
    ...
    if (k < page->hdr.lowest) {  //在只有一个root节点，既作root又作leaf的情况下，hdr.highest和hdr.lowest均是该类型的最大和最小值，因此均不符合
      assert(false);
      return false;
    }
...
//以上目前猜测是，成功执行了root分裂，并进行root的更新，所以返回了；下面执行的逻辑是，既需要进行node_split，同时并不是在root节点发生
