...
AbstractMessageConnection::AbstractMessageConnection(  //首先注册messagePool并且划分前半段为recvPool，后半段为sendPool
  ...
  send_cq = ibv_create_cq(ctx.ctx, 128, NULL, NULL, 0);  //创建send_cq

  createQueuePair(&message, type, send_cq, cq, &ctx);  //一个是send_cq，一个是recv_cq，并且创建与send_cq和recv_cq绑定的qp，其中send_cq属于Ab_connection自己
  modifyUDtoRTS(message, &ctx);                        //recv_cq则是传入的，因此可能与ThreadConnection以及Dirconnection有关
  //在这里经过的转换是干啥的呢？
