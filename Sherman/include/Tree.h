...
struct SearchResult {
  bool is_leaf;
  uint8_t level;  //每个leaf被创建的初期，level都被初始化为0
  GlobalAddress slibing;
  GlobalAddress next_level;
  Key upper_key;   //1+1+8+8+8+12+8+12=18+40=58个字节，padding的意义是啥？
...
public:
  Tree(DSM *dsm, uint16_t tree_id = 0);//这里规定了tree_id默认为0
...
private:
  DSM *dsm;
  uint64_t tree_id;
  GlobalAddress root_ptr_ptr; // the address which stores root pointer;存储指向root的指针
...
public:
  Key key;
  char key_padding[KEY_PADDING];
  GlobalAddress ptr;  //这里的GlobalAddress是指当前entry到远端节点的地址吗？
...
public:
  uint8_t f_version : 4;
  Key key;
  char key_padding[KEY_PADDING];  //这个padding可能是因为加入了k_v的f&r_version导致的填充 1+12+8+8+392
  Value value;
  char value_padding[VALUE_PADDING];  //PADDing的意义在哪呢？
  uint8_t r_version : 4;
...
constexpr int kLeafCardinality =
    (kLeafPageSize - sizeof(Header) - sizeof(uint8_t) * 2 - 8) / sizeof(LeafEntry);//大概是一个4的数字
