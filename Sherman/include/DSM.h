...
//  void initRDMAConnection_Memory();
  ...
  DSMConfig conf;
  std::atomic_int appID;  //这个appID是用来形成thread_id的
  Cache cache;
...
//这是fine_grained的管理方式，按照当前nodeID和threadID的情况，同一CN上的不同线程将依次发起对远端MN的alloc请求

//    printf("Allocate a new page");
  bool need_chunk = false;
  auto addr = local_allocator.malloc(size, need_chunk);  //need_chunk表明分配失败
  if (need_chunk) {
    RawMessage m;
    m.type = RpcType::MALLOC;

    this->rpc_call_dir(m, next_target_node, next_target_dir_id);//向远端的target_node的dir_id发出分配内存的message
    //而远端收到以后会在本地分配chunk，并返回chunk的Gloabl_Address
    local_allocator.set_chunck(rpc_wait()->addr);
    //rpc_wait会从cq里接收来自远端的message，其中包含新分配的chunk的global_address
    //set_chunk的本质是将获得的远端内存的Gloabl_address加入到本地内存分配器的log_heads里，表明thread_local的localAllocator拥有这块chunk

    if (++next_target_dir_id == NR_DIRECTORY) {
      next_target_node = (next_target_node + 1) % conf.MemoryNodeNum;
      next_target_dir_id = 0;//因为next_target_dir_id越界
    }  //但是这里要递增是为什么呢？
    //应该是进行下一次的malloc活动，在换成新的情况后，进行malloc的尝试
    //那么为什么need_chunk要更换target_node和target_dir_id呢?
    //暂且当作无用代码进行忽略.
    //在bench上去掉了这块代码，然后发现不会影响什么，所以暂时当作无用代码处理
    
    
    // retry
    addr = local_allocator.malloc(size, need_chunk);
  }//似乎本地local_Allocator是用来做本地影子内存管理的，实际上还是会调用rpc来呼叫远端MN节点进行内存分配
  //或者说，LocalAllocator维护的log_heads表明当前的thread从远端获取的chunk，被以vector的形式保存
  //目前猜测本地内存分配器先进行假的分配，然后再按照分配成功与否对远端进行实际操作
